---
title: "Kaggle Competition:  Rental Inquiry Interest Prediction"
output: html_document
---

Last year, Kaggle introduced a competition that tasked entrants with correctly predicting New York City rental property interest based on a wide variety of different factors displayed online.  The raw data included everything from images shown online to pricing to actual listing descriptions and addresses.  As someone who takes great interest in this type of analysis, this project appealed to me from the very beginning.  Unfortunately, I quickly found that the type of analysis that I wanted to perform was computationally limited by the hardware I currently have available.  In the end, however, that was not a signficant problem and instead provided an additional challenge to force myself to think of aspect of data modeling from an efficiency standpoint rather than being purely result-driven.  

Over time, Kagglers found that some of the data (particularly related to image names and dates) was not indexed/encoded properly and was overly influenced by features containing hidden time-based properties.  As such, top scorers were able to further improve their scores by taking advantage of these properties that should not have been present in the data.  Given my relative inexperience in complex data modeling at the time of the competition's conclusion, the lack of processing power which led me to abandon certain known sources of influence, and my choice to simply use the data as it was meant to be rather than to use erroneously embedded time-based information, I was pleased with the results of this project (top 50% scoring at the time of submission).  

First and foremost, loading up the required packages
...
Initializing multiple CPU core processing
...
The photos and features fields from the original data contain more data and need to be cleaned prior to incorporating them into the main dataset.  The following script creates and executes an array of command strings that brings over all of the other data fields into the traindata dataframe.
...
Because each variable was imported as a list, they must all be converted to the proper data type
...
Beginning the process to extract key features from features field. Extracting features and normalizing their format.
...
Due to the storage size of the numerous pictures available, there will be no analysis performed on an image by image basis.  Instead, file names will be examined and number images available for each listing will be used as potential features.  Listing features will be analyzed in detail below.
...
There are over 1700 different features listed, many of which are equivalent to each other.  The first step is to identify the most common features.
...
Once the common features are listed, it's beneficial to aggregate very similar features into those. Example:  'centralair' and 'centralac' clearly indicate the presence of 'airconditioning'
...
Even when the simpler synonymous terms are aggregated, there are still hundreds of feature terms that are rarely used that could be condensed into one of the common features determined above.
...
Creating data frame that compares the 'string distance' of all uncommon features to all of the common ones.  Similar to the way a spell checker might work, string distance effectively measures the number of characters by which two strings differ.  Once the string distances are populated, a new field is created that indicates which (if any) of the main features falls within a specified scaled distance. The rare feature is converted to match the main feature with the smallest scaled string distance. Rare features that do not have a close match are removed. The entire process condenses 1700 potential factor variables into 100--much more computationally manageable.
...
There are a handful of listings where the price listed was exorbitant; most of those were improperly classified as rentals when they were in fact for sale.  Removed those listings from the dataset.  Further price analysis will follow later.
...
p <- p + scale_x_continuous(limits = c(min(traindata$longitude)-5, max(traindata$longitude)+5))
p <- p + scale_y_continuous(limits = c(min(traindata$latitude)-5, max(traindata$latitude)+5))
plot(p)
```

The mapping indicates that there are a few geographical outliers while a substantial number of the listings are clustered around a specific region.  Attempts to normalize outlier lat/long data by crossreferencing the building_id field did not provide any corrections. Outliers weren't removed, but had their lat/long readings converted to NA.
...
Created a grid that is divided into roughly 1/2 square mile regions and assigned each listing to one specific categorical grid code based on its lat/long.  The idea is to attempt to capture a "neighborhood" or regional feature.
...
The results of the shapiro test above indicate that prices are normally distributed.  There are numerous buildings with several listings in each one.  Calculating z-values for each listing that relates the price to other units in that specific building.  For buildings with a small number of listings, this z-score is assigned to 0.
...
Calculating various listing densities.  Creating features that determine local densities of similar listing types based on number of bedrooms and bathrooms.
...
Initial analysis of description field.  Calculating the character length and the relative frequency of capital letters in each description.
...
Sentiment analysis of listing description.  Each description is analyzed and quantified based on 8 different emotional sentiments.   
...
Very basic analysis of photos/names.  Stock imagery is commonly used and can sometimes be distinguished from images taken on a per-listing basis. The results of the code below indicate that the image names are all unique and therefore determining if/when stock photography was used is not possible without further image analysis that will not be performed here due to the collective size of the image files.  

It should be noted that the image names were found by other Kaggler's to have a profound effect on listing interest because of an inherent time feature built into the name.  
...
Splitting the creation time of the listings to create categorical variables based on month, day, and time of day that each listing was created.
...
Street and Display Address information and data normalization.  Because of the lack in uniformity in which addresses were entered, creating factor variables from each different address would be a computationally  difficult task--well beyond the means of my home computer--and likely would prove to be useless.  Normalizing the address fields would make that much more feasible.  Example:  "256 fifth st", "256 5th ST" and "256 5th str" all represent the same address but would be interpreted as 3 distinct listings before normalizing.
